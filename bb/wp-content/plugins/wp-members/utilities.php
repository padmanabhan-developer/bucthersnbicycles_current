 ...
 * @param  string $placeholder optional for setting a placeholder for the field 
 * @return string $str the field returned as a string
 */
function wpmem_create_formfield( $name, $type, $value, $valtochk=null, $class='textbox', $placeholder=null)
{
	switch( $type ) {

	case "checkbox":
		if( $class = 'textbox' ) { $class = "checkbox"; }
		$str = "<input name=\"$name\" type=\"$type\" id=\"$name\" value=\"$value\" " . wpmem_selected( $value, $valtochk, $type ) . " /><label>".$placeholder."</label>\n";
		break;
	...
	case "textarea":
		...
		if ($placeholder != null) {
			$str = "<textarea cols=\"20\" rows=\"5\" name=\"$name\" id=\"$name\" class=\"$class\" placeholder=\"$placeholder\" >$value</textarea>";	
		}
		else {
	...
	case "password":
		if($placeholder != null) {
			$str = "<input name=\"$name\" type=\"$type\" id=\"$name\" class=\"$class\" placeholder=\"$placeholder\" />\n";
		}
		else {
...
if (!function_exists('wpmem_get_raintpl_instance')):
	
function wpmem_get_raintpl_instance()
{
