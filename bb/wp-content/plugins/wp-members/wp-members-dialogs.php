...
{ 	
    ...
    $arr = array( __( 'Login:', 'wp-members' ), __( 'User Name', 'wp-members' ), 'text', 'log', __( 'Password', 'wp-members' ), 'password', 'pwd', 'login', __( 'Login:', 'wp-members' ), 'username', 'password' );
...
function wpmem_inc_memberlinks( $page = 'members' ) 
{
	...
	case 'register':	
		$str = '<p>' . sprintf( __( 'Welcome back,  %s', 'wp-members' ), $user_login ) . '</p>
			<ul>
	...
	case 'login':
		...
		$str = '<p>
		  	' . sprintf( __( 'Welcome back,  %s', 'wp-members' ), $user_login ) . '<br />
	...
	case 'status':
		$str ='<p>
			' . sprintf( __( 'Welcome back,  %s', 'wp-members' ), $user_login ) . '  | 
			<a href="' . $logout . '">' . __( 'click here to logout', 'wp-members' ) . '</a>
			</p>';
		break;
...
function wpmem_inc_registration( $toggle = 'new', $heading = '' )
{
	if ( WPMEM_OLD_FORMS != 1 ) { 
		...
		$str = wpmem_registration_bb_template($toggle, $heading);
	} else {
...
function wpmem_login_form( $page, $arr ) 
{
	if ( WPMEM_OLD_FORMS != 1 ) { 
		...
		$str = wpmem_login_bb_template($page, $arr);
	} else {
		include_once( 'wp-members-deprecated.php' );
...
//	$test = get_current_user_id();
	...
	$heading = ( !$heading ) ? apply_filters( 'wpmem_register_heading', __( 'Signup:', 'wp-members' ) ) : $heading;
...
//	$form .= '<div>No strings attached. We have just made it easy for you to take time playing with the configuration you like. Sign up and save it for later - or refer to your account when contacting us, and we can quickly see your preferred MK I configuration, and guide you.</div>';
	
	$form .= '<div>This is the information we have from you. Edit and click SAVE if needed.</div>';
...
//			<div class="div_text"><p class="noinput">' . $userdata->user_login . '</p></div>';
		...
		$form = $form . '
			<div class="div_text">
				<input name="log" type="text" value="' . $val . '" class="username" id="username" placeholder="User Name *" />
			</div>';
	...
	{ 
		...
		if( $wpmem_fields[$row][4] == 'y' && $do_row == true ) {
			...
			if( $wpmem_fields[$row][2] != 'tos' && $wpmem_fields[$row][3] != 'checkbox') {
				...
				$wpmem_fields[$row][1] = ( $wpmem_fields[$row][5] == 'y' ) ? $wpmem_fields[$row][1] . ' *' : $wpmem_fields[$row][1];
			...
			if ($wpmem_fields[$row][2] == 'city' || $wpmem_fields[$row][2] == 'zip')
			{
				array_push($cssClasses, 'wpmem-half-width');
			}
			...
			if( $wpmem_fields[$row][2] == 'tos' ) { 
				...
				$form = $form . wpmem_create_formfield( $wpmem_fields[$row][2], $wpmem_fields[$row][3], $wpmem_fields[$row][7], $val, '', $wpmem_fields[$row][1] );
				...
				if( $wpmem_fields[$row][3] == 'checkbox' ) { 
					...
					//label
					$wpmem_fields[$row][1] = ( $wpmem_fields[$row][5] == 'y' ) ? '<font class="req">*</font>' . $wpmem_fields[$row][1] : $wpmem_fields[$row][1];
				}
				...
				array_push($cssClasses, 'textbox');

				$form = $form . wpmem_create_formfield( $wpmem_fields[$row][2], $wpmem_fields[$row][3], $val, $valtochk, implode(' ', $cssClasses), $wpmem_fields[$row][1] );
			}
		...
		$form = $form . '<div class="req-text"><font class="req">*</font>' . __( 'We kindly ask you to provide this basic information for us to accurately respond to your request - thank you.', 'wp-members' ) . '</div>';
	...
	if( WPMEM_CAPTCHA == 1 && $toggle != 'edit' ) { // don't show on edit page!
		...
		if( $wpmem_captcha[0] && $wpmem_captcha[1] ) {
			...
			$form = $form . '<div class="clear"></div>
				<div style="float: right class="captcha">';
...
//		</div>';
	...
	$form = $form . '<input name="redirect_to" type="hidden" value="' . get_permalink() . '" />
		<div class="button_div">
			<div class="left">
				<input name="submit" type="submit" value="' . __( 'Save Profile', 'wp-members' ) . '" class="buttons buttons_bold left" />
			</div>
			<div class="right">
				<input name="delete" type="submit" value="' . __( 'Delete Account', 'wp-members' ) . '" class="buttons buttons_bold right"/>
			</div>
		</div>';
...
if (!function_exists('wpmem_registration_bb_template')):
	
	
/**
 * function with template for B&B
 * 
 */
function wpmem_registration_bb_template($toggle = 'new', $heading = '')
{
	$tpl = wpmem_get_raintpl_instance();
	...
	$heading = ( !$heading ) ? apply_filters( 'wpmem_register_heading', __( 'Signup:', 'wp-members' ) ) : $heading;
	$legend = ($toggle == 'new') 
			? 'No strings attached. We have just made it easy for you to take time playing with the configuration you like. Sign up and save it for later - or refer to your account when contacting us, and we can quickly see your preferred MK I configuration, and guide you.'
			: 'This is the information we have from you. Edit and click SAVE if needed.';
	
	$form = apply_filters( 'wpmem_register_form_before', '' );
	
	$form .= '[wpmem_txt]';
	
	$tpl->assign('permalink', get_permalink());
	$tpl->assign(
			'nonce_field', 
			(defined('WPMEM_USE_NONCE')) ? wp_nonce_field('wpmem-validate-submit', 'wpmem-form-submit') : ''
	);
	$tpl->assign('heading', $heading);
	$tpl->assign('legend', $legend);
	$tpl->assign('username', wpmem_tpl_assign_username($toggle));
	$tpl->assign('a_value', ( $toggle == 'edit' ) ? 'update' : 'register');
	$tpl->assign('permalink', get_permalink());
	$tpl->assign('delete_account', wpmem_tpl_assign_delete_button($toggle));
	
	wpmem_tpl_assign_fields($toggle, $tpl);
	...
	$form .= $tpl->draw('registration_form', true);
	$form = $form . '[/wpmem_txt]';
...
function wpmem_tpl_assign_delete_button($toggle)
{
	$delete_btn = '';
	if ($toggle == 'edit') {
		$delete_btn .= '<input name="delete" id="delete_user_account_2" type="submit" value="Delete Account" class="buttons buttons_bold right"/>';
		$delete_btn .= '<div class="delete_user_message" style="display: none;"></div>';
	}
	
	return $delete_btn;
}

function wpmem_tpl_assign_fields($toggle, $tpl) {
	global $wpmem_regchk, $userdata;
	$wpmem_fields = get_option('wpmembers_fields');
	$test = "";
	for ($row = 0; $row < count($wpmem_fields); $row++) {
		...
		if ($wpmem_fields[$row][4] == 'y' && $do_row == true) {
			...
			if ($wpmem_fields[$row][2] == 'tos') {
				...
				if ($wpmem_fields[$row][3] == 'checkbox') {
					...
					$val = $wpmem_fields[$row][7];
					// if it should it be checked by default (& only if form not submitted), then override above...
					if ($wpmem_fields[$row][8] == 'y' && (!$_POST && $toggle != 'edit' )) {
						$val = $valtochk = $wpmem_fields[$row][7];
					}
					//label
					$wpmem_fields[$row][1] = ( $wpmem_fields[$row][5] == 'y' ) ? '<font class="req">*</font>' . $wpmem_fields[$row][1] : $wpmem_fields[$row][1];
				}
		...
		if ($wpmem_fields[$row][2] != '') {
			$tpl->assign($wpmem_fields[$row][2], $form);
			$test .= $wpmem_fields[$row][2] . ' ';
		}
...
function wpmem_tpl_assign_tos($wpmem_fields, $toggle, $do_row)
{
	global $userdata;
	...
	if( $wpmem_fields[$row][2] == 'tos' && $toggle == 'edit' && ( get_user_meta($userdata->ID, 'tos', true ) ) ) { 
		// makes tos field hidden on user edit page, unless they haven't got a value for tos
		$do_row = false; 
		$tos = wpmem_create_formfield( $wpmem_fields[$row][2], 'hidden', get_user_meta( $userdata->ID, 'tos', true ));
	}
	
	return $tos;
}

function wpmem_tpl_assign_username($toggle='new')
{
	$form = '';
...
if (!function_exists('wpmem_login_bb_template')):
	
function wpmem_login_bb_template($page, $arr) 
{
	...
	$tpl->assign('permalink', get_permalink());
	$tpl->assign('legend', $arr[0]);
	$tpl->assign('login', wpmem_create_formfield( $arr[3], $arr[2], '', '', $arr[9], $arr[1] ));
	$tpl->assign('password', wpmem_create_formfield( $arr[6], $arr[5], '', '', $arr[10], $arr[4] ));
	$tpl->assign('redirect_to', $redirect_to);
	$tpl->assign('login_buttons', wpmem_assign_login_buttons($arr));
	
	$form .= $tpl->draw('login_form', true);
	$form .= '[/wpmem_txt]';
...
function wpmem_assign_login_buttons($arr)
{
	$form = '';
	if ( $arr[7] != 'login' ) 
	{ 
	...
	$form = $form . '<div class="button_div_wrapper">';
	
	if ( $arr[7] == 'login' ) 
	{
		$link = apply_filters( 'wpmem_forgot_link', wpmem_chk_qstr( WPMEM_MSURL ) . 'a=pwdreset' );	
		$form = $form . '<div class="wpmem-pass-reset">' . __('Forgot your ', 'wp-members') . '&nbsp;<a href="' . $link . '">' . __('password', 'wp-members') . '</a>?</div>';
		$form = $form . '<div class="button_div"><input name="rememberme" type="checkbox" id="rememberme" value="forever" />&nbsp;' . __('Remember me', 'wp-members') . '&nbsp;&nbsp;<input type="submit" name="Submit" value="' . $arr[8] . '" class="buttons" />';
	} 
	else 
	{
...
function wpmem_login_form_NEW( $page, $arr ) 
{
	...
	$form.= '[wpmem_txt]<div id="wpmem_login">
		<form action="' . get_permalink() . '" method="POST" class="form">
			...
			<div class="div_text">
				' . wpmem_create_formfield( $arr[3], $arr[2], '', '', $arr[9], $arr[1] ) . '
			</div>
			
			<div class="div_text">
				' . wpmem_create_formfield( $arr[6], $arr[5], '', '', $arr[10], $arr[4] ) . '
			</div>
...
function wpmem_page_user_edit( $wpmem_regchk, $content )
{
	...
	$heading = apply_filters( 'wpmem_user_edit_heading', __( 'Edit Profile', 'wp-members' ) );
